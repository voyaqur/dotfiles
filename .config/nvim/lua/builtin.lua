...
vim.diagnostic.config({
	...
	severity_sort = true,
	underline = true,
	update_in_insert = false, -- don't flash red text while mid-typing
	...
	float = {
		...
		prefix = function(diagnostic, i, total)
			local info = diagnostic_severity_info[diagnostic.severity]
			local icon = info and info.icon or "●"
			local hl = info and info.hl or "Normal"
			-- local source = diagnostic.source and string.format("[%s] ", diagnostic.source) or ""
			return string.format("%d/%d %s ", i, total, icon), hl
		end,
